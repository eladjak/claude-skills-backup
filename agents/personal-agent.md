...
APPLY:   יצירת/עדכון skills → שיפור agents → שדרוג rules
   ↓
GROW:    מה עשיתי טוב יותר? על מה אדחוף בפעם הבאה?
   ↓
SHARE:   הפצת תובנות לכל הסוכנים ברשת
   ↓
חזרה ל-OBSERVE
```

### עקרון הצמיחה האקטיבית (ראה: `~/.claude/rules/agent-growth-directive.md`)
- לא רק לבצע משימות — לדחוף להיות טוב יותר בכל איטרציה
- לא לחכות להוראות לשפר — לזהות הזדמנויות ולפעול
- כל תובנה → שדרוג skill, כלל, או הוראות סוכן
- לשתף תובנות עם כל הסוכנים ברשת (Claude Code, Kaylee, OMC)
- לנסות גישות חדשות, לתעד תוצאות, לאמץ מה שעובד
